...
// 7. Create System User (ID 0)
echo "\n--- 7. System User (ID 0) Check ---\n";
// Enable ID 0 insertion
mysqli_query($conn, "SET SESSION sql_mode='NO_AUTO_VALUE_ON_ZERO'");
$check_sys = mysqli_query($conn, "SELECT id_nethera FROM nethera WHERE id_nethera = 0");
if (mysqli_num_rows($check_sys) > 0) {
    ...
    if (mysqli_num_rows($check_email) > 0) {
        ...
        $sql_sys = "INSERT INTO nethera (id_nethera, nama_lengkap, username, password, email, role, status_akun, gold) 
                VALUES (0, 'System Administrator', 'System', '$pass', 'system@moe.local', 'Vasiki', 'Aktif', 999999)";
        if (mysqli_query($conn, $sql_sys)) {
            echo "✅ Created System User (ID 0).\n";
        } else {
