...
function getBattlePetImage(pet) {
    ...
    for (const key of imgKeys) {
        if (pet[key] && pet[key] !== '' && pet[key] !== null) {
            return `${ASSET_BASE}assets/pets/${pet[key]}`;
        }
    }

    // Fallback to placeholder
    return `${ASSET_BASE}assets/placeholder.png`;
}
...
async function loadBattleState() {
    try {
        const response = await fetch(`${API_BASE}battle/state?battle_id=${BattleState.battleId}`);
        const data = await response.json();

        if (!data.success) {

            alert('Battle not found or expired!');
            window.location.href = ASSET_BASE + 'pet?tab=arena3v3';
            return;
        }

        const state = data.battle_state;
        if (!state || !state.player_pets || !state.enemy_pets) {
            console.error('Invalid battle state structure received:', state);
            alert('Battle state is invalid or incomplete!');
            return;
        }

        BattleState.playerPets = state.player_pets;
        BattleState.enemyPets = state.enemy_pets;
        BattleState.activePlayerIndex = state.active_player_index !== undefined ? state.active_player_index : 0;
        BattleState.activeEnemyIndex = state.active_enemy_index !== undefined ? state.active_enemy_index : 0;
        BattleState.currentTurn = state.current_turn || 'player';
        BattleState.turnCount = state.turn_count || 1;
        BattleState.status = state.status || 'active';
...
async function loadPlayerSkills() {
    ...
    if (!activePet || !activePet.skills) {
        console.warn('No skills found for active pet, using defaults');
        BattleState.playerSkills = getDefaultSkills(activePet ? activePet.element : 'Fire');
        return;
    }
    BattleState.playerSkills = activePet.skills;
}
...
async function handleAttack(skillId) {
    ...
    try {
        ...
        const response = await fetchWithCsrf(`${API_BASE}battle/attack`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                battle_id: BattleState.battleId,
        ...
        data.logs.forEach(log => {
            ...
            if (data.element_advantage === 'super_effective') logClass += ' effective';
            if (data.element_advantage === 'not_effective') logClass += ' weak';
        ...
        BattleState.currentTurn = battleState.current_turn || 'enemy';
        BattleState.turnCount = battleState.turn_count || BattleState.turnCount || 1;
        BattleState.status = battleState.status || 'active';
...
async function enemyTurn() {
    ...
    try {
        ...
        const response = await fetchWithCsrf(`${API_BASE}battle/enemy-turn`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                battle_id: BattleState.battleId
        ...
        // Update state from server response
        const battleState = data.battle_state;
        BattleState.currentTurn = battleState.current_turn || 'player';
        BattleState.turnCount = battleState.turn_count || BattleState.turnCount || 1;
        BattleState.status = battleState.status || 'active';
        BattleState.activePlayerIndex = battleState.active_player_index;
        BattleState.activeEnemyIndex = battleState.active_enemy_index;
...
function renderTeamIndicators() {
    ...
    DOM.playerIndicators.innerHTML = BattleState.playerPets.map((pet, i) => {
        ...
        <div class="pet-indicator ${i === BattleState.activePlayerIndex ? 'active' : ''} ${pet.is_fainted ? 'fainted' : ''}">
            <img src="${getBattlePetImage(pet)}" alt="${pet.species_name || 'Pet'}" onerror="this.onerror=null; this.src='../assets/placeholder.png'">
        </div>
    `;
    }).join('');
...
function renderActivePets() {
    ...
    // Player
    DOM.playerName.textContent = playerPet.name || playerPet.nickname || playerPet.species_name || 'Unknown';
    ...
    // Enemy
    DOM.enemyName.textContent = enemyPet.name || enemyPet.nickname || enemyPet.species_name || 'Unknown';
...
function renderSkills() {
    ...
    DOM.skillsGrid.innerHTML = skills.slice(0, 4).map(skill => `
        <button class="move-btn ${(skill.skill_element || activePet.element || 'fire').toLowerCase()}" 
                onclick="handleAttack(${skill.id})"
                ${BattleState.currentTurn !== 'player' ? 'disabled' : ''}>
            <div class="move-name">${skill.skill_name || 'Attack'}</div>
            <div class="move-stats">
              <span class="cost-badge"><i class="fas fa-bolt"></i> ${skill.base_damage || 25}</span>
            </div>
        </button>
    `).join('');
}
...
function updatePlayerHp(hp, isFainted) {
    ...
    if (percent < 30) {
        DOM.playerHpBar.classList.add('low-health');
        if (DOM.panelPlayerHp) DOM.panelPlayerHp.classList.add('low-health');
    } else {
        DOM.playerHpBar.classList.remove('low-health');
        if (DOM.panelPlayerHp) DOM.panelPlayerHp.classList.remove('low-health');
    }
}
...
function updateEnemyHp(hp, isFainted) {
    ...
    if (percent < 30) {
        DOM.enemyHpBar.classList.add('low-health');
        if (DOM.panelEnemyHp) DOM.panelEnemyHp.classList.add('low-health');
    } else {
        DOM.enemyHpBar.classList.remove('low-health');
        if (DOM.panelEnemyHp) DOM.panelEnemyHp.classList.remove('low-health');
    }
}
...
function showFloatingDamage(targetElement, damage, isCritical, advantage) {
    ...
    if (isCritical) {
        ...
        // Trigger screen shake and hit flash for critical hits
        triggerScreenShake();
        triggerHitFlash();
...
function triggerScreenShake() {
    const stage = document.querySelector('.battle-container');
    if (!stage) return;
...
/**
 * Trigger bright hit flash overlay
 */
...
function openSwapModal() {
    ...
    DOM.swapPets.innerHTML = BattleState.playerPets.map((pet, i) => {
        ...
        <div class="swap-pet-option ${i === BattleState.activePlayerIndex ? 'current' : ''} ${isFainted ? 'fainted' : ''}"
            ...
            <img src="${getBattlePetImage(pet)}" alt="${petName}" class="swap-pet-img" onerror="this.onerror=null; this.src='../assets/placeholder.png'">
...
async function switchToPet(index) {
    ...
    try {
        ...
        const response = await fetchWithCsrf(`${API_BASE}battle/switch-pet`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                battle_id: BATTLE_ID,
...
async function endBattle(playerWon) {
    if (BattleState.status === 'processing_end') return;
    BattleState.status = 'processing_end';
    disableControls(true);

    // Call API to finalize battle on server
    let goldReward = 0;
    let expReward = 0;
    let eloChange = 0;
    let petsDamaged = [];
    ...
    const data = await apiCall('battle/finish-3v3');
    if (data && data.success) {
        goldReward = data.reward_gold || 0;
        expReward = data.reward_exp || 0;
