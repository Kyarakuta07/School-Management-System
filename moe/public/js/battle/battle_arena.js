...
// ================================================
const _cfgEl = document.getElementById('battle-config');
const _d = _cfgEl ? _cfgEl.dataset : {};

const BATTLE_CONFIG = {
    attackerPetId: parseInt(_d.attackerPetId) || 0,
    defenderPetId: parseInt(_d.defenderPetId) || 0,
    attackerElement: _d.attackerElement || 'fire',
    defenderElement: _d.defenderElement || 'fire',
    attackerMaxHp: parseInt(_d.attackerMaxHp) || 100,
    defenderMaxHp: parseInt(_d.defenderMaxHp) || 100,
    attackerLevel: parseInt(_d.attackerLevel) || 1,
    defenderLevel: parseInt(_d.defenderLevel) || 1,
    attackerBaseAtk: parseInt(_d.attackerBaseAtk) || 50,
    defenderBaseAtk: parseInt(_d.defenderBaseAtk) || 50,
    attackerBaseDef: parseInt(_d.attackerBaseDef) || 50,
    defenderBaseDef: parseInt(_d.defenderBaseDef) || 50,
    attackerRarity: _d.attackerRarity || 'Common',
    defenderRarity: _d.defenderRarity || 'Common',
    attackerEvolution: _d.attackerEvolution || 'egg',
    defenderEvolution: _d.defenderEvolution || 'egg',
    defenderSkills: _d.defenderSkills ? JSON.parse(_d.defenderSkills) : [],
    csrfToken: _d.csrfToken || '',
    csrfHeader: _d.csrfHeader || '',
    battleType: _d.battleType || 'arena',
    warId: parseInt(_d.warId) || 0
};
const API_BASE = _d.apiBase || '/api/';
const ASSET_BASE = _d.assetBase || '/';
...
async function useSkill(skillId, baseDamage, skillElement) {
    ...
    // Process turn-start status effects (burn DOT, stun check, etc.)
    const turnEffects = processTurnStartEffects('player');
    if (turnEffects.logs.length > 0) {
        turnEffects.logs.forEach(log => addBattleLog(log, 'status-effect'));
    }
    if (turnEffects.damage > 0) {
        BattleState.playerHp = Math.max(0, BattleState.playerHp - turnEffects.damage);
        updateHpDisplay();
        showDamagePopup(DOM.playerSprite, turnEffects.damage, 1.0, false);
        if (BattleState.playerHp <= 0) { endBattle(false); return; }
    }
    if (!turnEffects.canAct) {
        addBattleLog(`⚡ You are ${turnEffects.blockReason}! Turn skipped.`, 'status-effect');
    ...
    try {
        ...
        const response = await fetchWithCsrf(`${API_BASE}battle/attack-1v1`, {
            method: 'POST',
        ...
        // If status effect was applied to enemy by this skill
        if (data.status_applied && data.status_effect) {
            const cfg = STATUS_CONFIG[data.status_effect];
            addStatusEffect('enemy', {
                type: data.status_effect,
                turns_left: data.status_duration || 2,
                icon: cfg?.icon,
                name: cfg?.name
            });
            addBattleLog(`${cfg?.icon || '❓'} Enemy is inflicted with ${cfg?.name || data.status_effect}!`, 'status-effect');
        } else if (data.status_resisted) {
            addBattleLog(`🛡️ Enemy resisted the status effect!`, 'status-resist');
        }
...
async function enemyTurn() {
    ...
    // Process turn-start status effects for enemy (burn DOT, stun check, etc.)
    const turnEffects = processTurnStartEffects('enemy');
    if (turnEffects.logs.length > 0) {
        turnEffects.logs.forEach(log => addBattleLog(log, 'status-effect'));
    }
    if (turnEffects.damage > 0) {
        BattleState.enemyHp = Math.max(0, BattleState.enemyHp - turnEffects.damage);
        updateHpDisplay();
        showDamagePopup(DOM.enemySprite, turnEffects.damage, 1.0, false);
        if (BattleState.enemyHp <= 0) { endBattle(true); return; }
    }
    if (!turnEffects.canAct) {
        addBattleLog(`⚡ Enemy is ${turnEffects.blockReason}! Turn skipped.`, 'status-effect');
        await new Promise(r => setTimeout(r, 800));
        restorePlayerTurn();
        return;
    }
    ...
    try {
        ...
        const response = await fetchWithCsrf(`${API_BASE}battle/enemy-turn-1v1`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                attacker_pet_id: BATTLE_CONFIG.attackerPetId,
        ...
        // If enemy's skill applied status effect to player
        if (data.status_applied && data.status_effect) {
            const cfg = STATUS_CONFIG[data.status_effect];
            ...
            addBattleLog(`🛡️ Your pet resisted the status effect!`, 'status-resist');
        }
...
function updateHpDisplay() {
    ...
    if (playerPercent < 30) {
        ...
        DOM.playerHpBar.style.boxShadow = '0 0 20px rgba(255, 0, 0, 0.5)';
    } else {
        DOM.playerHpBar.classList.remove('low');
        DOM.playerHpBar.style.boxShadow = '';
    }
    ...
    if (enemyPercent < 30) {
        ...
        DOM.enemyHpBar.style.boxShadow = '0 0 20px rgba(255, 0, 0, 0.5)';
    } else {
        DOM.enemyHpBar.classList.remove('low');
        DOM.enemyHpBar.style.boxShadow = '';
    }
}
...
function showDamagePopup(targetElement, damage, multiplier, isCrit) {
    ...
    if (isCrit) {
        ...
        // Trigger cinematic effects for critical hits
        triggerScreenShake();
        triggerImpactFlash(targetElement);
    } else if (multiplier > 1) {
        fct.classList.add('effective');
        fct.textContent = `${damage}!`;
        // Slight screen shake for effective hits
        if (damage > 20) triggerScreenShake();
        triggerImpactFlash(targetElement, 0.5); // Half intensity flash
    } else if (multiplier < 1) {
...
/**
 * Trigger high-intensity flash on target when hit
 */
function triggerImpactFlash(target, opacity = 1) {
    if (!target) return;

    // Add impact flash class to target sprite
    target.classList.add('impact-flash');
    if (opacity < 1) target.style.opacity = opacity;
...
function disableSkills(disabled) {
    ...
    buttons.forEach(btn => {
        ...
        // Remove glints when disabled
        if (disabled) btn.classList.remove('skill-ready-glint');
    });
}

/**
 * Phase 2: Apply glint effect to available skills
 */
function applySkillGlints(apply) {
    const buttons = DOM.skillsPanel.querySelectorAll('.skill-btn');
    buttons.forEach(btn => {
        if (apply && !btn.disabled) {
            btn.classList.add('skill-ready-glint');
        } else {
            btn.classList.remove('skill-ready-glint');
        }
    });
}

/**
 * Phase 2: Cinematic turn-start camera zoom
 */
function triggerCameraZoom() {
    const stage = document.querySelector('.battle-stage');
    if (!stage) return;

    stage.classList.add('camera-zoom-in');
    setTimeout(() => {
        stage.classList.remove('camera-zoom-in');
    }, 800);
}
...
function endBattle(playerWon) {
    ...
    // Submit result to backend FIRST, then show result with server values
    submitBattleResult(playerWon);
}

async function submitBattleResult(playerWon) {
    try {
        // DIFFERENTIAL LOGIC FOR SANCTUARY WAR
        if (typeof BATTLE_CONFIG !== 'undefined' && BATTLE_CONFIG.battleType === 'war') {
            const response = await fetchWithCsrf(`${API_BASE}war/finalize`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    war_id: BATTLE_CONFIG.warId,
                    opponent_pet_id: BATTLE_CONFIG.defenderPetId,
                    won: playerWon
                })
            });
            const data = await response.json();
            if (data.success) {
                // Return to war tab with state
                window.location.href = `${ASSET_BASE}pet?tab=war&battle_won=${playerWon}`;
                return;
            }
        }

        const response = await fetchWithCsrf(`${API_BASE}battle/result`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                attacker_pet_id: BATTLE_CONFIG.attackerPetId,
        ...
        // Use server-computed rewards (authoritative)
        const goldReward = data.success ? (data.reward_gold || 0) : 0;
        const expReward = data.success ? (data.reward_exp || 0) : 0;
        const eloChange = data.success ? (data.elo_change || 0) : 0;

        // Show result overlay with SERVER values
        ...
        const rpEl = document.getElementById('reward-rp');
        if (rpEl) {
            rpEl.textContent = (eloChange >= 0 ? '+' : '') + eloChange;
        }

        DOM.resultOverlay.classList.remove('hidden');

        // Show battle log details
        if (eloChange !== 0) {
            addBattleLog(`🛡️ Rank Points: ${eloChange >= 0 ? '+' : ''}${eloChange}`, eloChange >= 0 ? 'bonus' : 'error');
        }
        ...
        // Fallback: show result overlay even if server fails
        const resultTitle = document.getElementById('result-title');
        if (playerWon) {
            resultTitle.textContent = '🏆 Victory!';
            resultTitle.className = 'victory';
...
async function forfeitBattle() {
    ...
    if (confirm('Forfeit this battle? You will lose!')) {
        ...
        addBattleLog('🏳️ Forfeiting battle...', 'error');
        disableSkills(true);

        // Mark as over to prevent further actions or enemy turns
        BattleState.isBattleOver = true;

        try {
            // Submit loss to backend — this shows the defeat result overlay
            await submitBattleResult(false);
        } catch (error) {
            console.error('Forfeit failed:', error);
            window.location.assign(ASSET_BASE + 'pet?tab=arena');
        }
    }
}
