...
export async function loadInventory() {
    try {
        const response = await fetchWithCsrf(`${API_BASE}shop/inventory`);
...
export async function openItemModal(type) {
    ...
    // Set appropriate icon in header
    const headerIcon = document.getElementById('item-modal-icon-header');
    if (headerIcon) {
        headerIcon.className = type === 'food' ? 'fas fa-utensils' : 'fas fa-flask';
    }
        ...
        return `
            <div class="dead-pet-card" onclick="revivePet(${pet.id})">
                <img src="${imgPath}" alt="${displayName}" 
                     onerror="this.onerror=null; this.src='${ASSETS_BASE}../placeholder.png'">
