...
async function setActivePet(petId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            window.location.href = `${ASSET_BASE}pet?tab=my-pet`;
        } else {
...
function openPetDetailById(petId) {
    ...
    const pet = window.userPets.find(p => p.id == petId); // Loose equality for type coercion
...
function openPetDetail(pet) {
    ...
    if (!modal) {
        console.warn('Pet detail modal not found in DOM');
        return;
    }

    // Get image path
    const imgPath = getPetImagePath(pet);
    const isShiny = pet.is_shiny == 1 || pet.is_shiny === true;
    ...
    // Shiny tag
    document.getElementById('detail-shiny-tag').style.display = isShiny ? 'flex' : 'none';
...
async function setActiveFromDetail() {
    ...
    try {
        ...
        if (data.success) {
            showToast('Pet is now active!', 'success');
            window.loadActivePet(); // Reload to see updated state
            closePetDetail();
            // Redirect to my-pet tab with page refresh for consistent behavior
            window.location.href = `${ASSET_BASE}pet?tab=my-pet`;
        } else {
